# Task Management API (Spring Boot + AWS)

JWT認証付きのタスク管理REST APIです。

Dockerによるローカル環境構築と、
AWS（EC2 + RDS）へのデプロイに対応しています。

認証・認可・データ分離を実装し、
実務を意識したバックエンド構成になっています。
...
本アプリケーションはAWS上で動作します。

- EC2上でDockerコンテナとしてSpring Bootを実行
- RDS（PostgreSQL）をデータベースとして利用

### 構成

Client → EC2 (Docker) → RDS
...
http://localhost:8080/swagger-ui/index.html（ローカル環境）
